...
for(j in 1:5){
  ...
  c <- c-1
  pcc <- c(1, 2, 3, 4, 5, 6, 7, 8, 9)
  names <- switch(j, "Объем продаж", "Списания", "Прибыль", "Объём продаж", "Рентабельность")
  plot(x = line[1:num], type = "o", col = "red", xlab = "Дни", ylab = names, pch = c(1, 16))
  for (i in range(2:length(goods))) {
    lines(x = line[((i-1)*num):(i*num)],type="o", col=colors[i], pch=pcc[i])
    legend("topleft", legend=goods[1:3], col=colors[1:3], pch=pcc[i])
...
for (i in 1:10){
  ...
  if (i==1) { # Анализ товара
    plot(x = v, type = "o", col = "red", xlab = "Дни", ylim =c(0,100 * 1.5), ylab = paste0("Объем продаж, шт. Товар: ",goods[i]), pch = pcc[i])
    grid(col="lightblue", nx=30)
  }
  if (i==10) {
    for(j in  2:10) {
      lines(x = v[((j-1)*length(v)/10):(j*length(v)/10)], type = "o", col = colors[j], xlab = "Дни", ylab = paste0("Объем продаж, шт. Товар: ", goods[i]))
      legend("topright", legend=marketplaces.names, col=colors, pch=c(1, 16))
    }
  }
}
...
for (i in 1:10) {
  assign(paste0("input", as.character(i)), read.table(paste0("~/DEV/R_projects/Market/Marketplace_",as.character(i),"/import_all.in"), head=TRUE))
  assign(paste0("output", as.character(i)), read.table(paste0("~/DEV/R_projects/Market/Marketplace_",as.character(i),"/export_all.out"), head=TRUE))
  v <- append(v, eval(parse(text = paste0("input", as.character(i)))[,goods[2]]))
  v <- append(v, eval(parse(text = paste0("output", as.character(i)))[,goods[2]]))

  if (i==1) {
    plot(x = v[1:(length(v)/2)], type = "o", col = colors[i], xlab = "Дни",ylim =c(0,80), ylab = paste0("Объем продаж, шт. Товары: ",goods), pch = c(1, 16))
    print(i)
  ...
  if(i==10){
    for(j in  2:10){
      ...
      legend("topright", legend=c(goods[1],goods[2],goods[3],goods[4],goods[5],goods[6]))
